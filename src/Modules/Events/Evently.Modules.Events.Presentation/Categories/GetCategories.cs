...
internal static class GetCategories
{
    public static void MapEndpoint(IEndpointRouteBuilder app)
    {
        ...
        {
            var categories = await cacheService.GetAsync<List<CategoryResponse>>("categories");

            if (categories is not null) return Results.Ok(categories);

            Result<List<CategoryResponse>> result = await sender.Send(new GetCategoriesQuery());

            if (result.IsSuccess) await cacheService.SetAsync("categories",  result.Value);
